...
class RawFeatureGraph:
    ...
    Attributes:
        ...
        old_cfg
        g(networkx.Digraph)
        entry(int): entry addr  入口
    """
    def __init__(self, func_name, img, entry):
        self.img = img
        self.func_name = func_name
        self.old_cfg = self.img.get_cfg(func_name)  # 获取函数对应的CFG
        self.g = nx.DiGraph()  # 创建一个没有节点和边的空图
        self.entry = entry
        self.init_graph()
...
def main():
    ...
    img = Image(bin1)
    fname = list(img.funcs)  # 存储img中所有的函数,并将数据格式由set转为list
